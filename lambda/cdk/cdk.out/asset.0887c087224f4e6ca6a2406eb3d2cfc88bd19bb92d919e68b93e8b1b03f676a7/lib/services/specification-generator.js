...
class SpecificationGenerator {
     ...
     */
    async extractInformation(userMessage, assistantResponse, context) {
        ...
        if (lowerUser.includes('integrate') ||
     ...
     */
    async updateSpecification(sessionId, extractedInfo, currentSpecification) {
        // Extract the topic and data
        const { topic, data } = extractedInfo;
        // Update plain English summary
        const updatedSummary = this.updatePlainEnglishSummary(currentSpecification.plainEnglishSummary, topic, data);
        // Update formal PRD (pass empty array for conversation history since we don't have it here)
        const updatedPRD = await this.updateFormalPRD(currentSpecification.formalPRD, topic, data, []);
        // Increment version
     ...
     */
    async generateFormalPRD(userIntent, conversationHistory) {
        ...
        const nonFunctionalRequirements = this.generateNonFunctionalRequirements(userIntent, conversationHistory);
        return {
            introduction,
            glossary,
            requirements,
     ...
     */
    async generatePlainEnglishSummary(formalPRD) {
     ...
     */
    validateCompleteness(specification) {
        const missingTopics = [];
     ...
     */
    updatePlainEnglishSummary(currentSummary, topic, data) {
     ...
     */
    async updateFormalPRD(currentPRD, topic, data, conversationHistory) {
     ...
     */
    extractIntroduction(conversationHistory, userIntent) {
        // Find the first substantial user message
        const firstUserMessage = conversationHistory.find((msg) => msg.role === 'user' && msg.content.length > 20);
        if (firstUserMessage) {
     ...
     */
    buildGlossary(conversationHistory, userIntent) {
        const glossary = {};
     ...
     * Generate requirements from user intent
     */
    generateRequirements(userIntent, conversationHistory) {
        const requirements = [];
        // Generate requirements from features
        if (userIntent.features && Array.isArray(userIntent.features)) {
            userIntent.features.forEach((feature, index) => {
        ...
        if (userIntent.workflows && Array.isArray(userIntent.workflows)) {
            userIntent.workflows.forEach((workflow, index) => {
                const requirement = this.createRequirementFromWorkflow(workflow, requirements.length + index + 1);
                requirements.push(requirement);
            });
        }
        return requirements;
     ...
     * Generate non-functional requirements
     */
    generateNonFunctionalRequirements(userIntent, conversationHistory) {
        const nfrs = [];
        ...
        if (userIntent.nonFunctionalRequirements && Array.isArray(userIntent.nonFunctionalRequirements)) {
            userIntent.nonFunctionalRequirements.forEach((nfr, index) => {
                nfrs.push({
     ...
     */
    createRequirementFromFeature(feature, index) {
     ...
     */
    createRequirementFromWorkflow(workflow, index) {
        return {
            id: `req-${index}`,
     ...
     */
    translateBusinessToTechnical(businessGoal) {
     ...
     * Extract overview from introduction
     */
    extractOverviewFromIntroduction(introduction) {
     ...
     * Extract key features from requirements
     */
    extractKeyFeaturesFromRequirements(requirements) {
        return requirements
     ...
     * Extract target users from requirements
     */
    extractTargetUsersFromRequirements(requirements) {
     ...
     * Extract integrations from requirements
     */
    extractIntegrationsFromRequirements(requirements) {
     ...
     */
    identifyCoveredTopics(specification) {
        const covered = [];
        ...
        const hasDataRequirements = specification.formalPRD.requirements.some((req) => req.userStory.toLowerCase().includes('data') ||
            req.acceptanceCriteria.some((ac) => ac.toLowerCase().includes('data')));
        if (hasDataRequirements) {
            covered.push('data');
        }
        // Check for workflows
        const hasWorkflows = specification.formalPRD.requirements.some((req) => req.userStory.toLowerCase().includes('workflow') ||
    ...
    detectConflicts(requirements) {
        ...
        for (let i = 0; i < requirements.length; i++) {
            for (let j = i + 1; j < requirements.length; j++) {
                ...
                if ((text1.includes('not') && text2.includes('shall')) ||
     ...
     */
    generateIntroductionFromData(data) {
        if (data.description) {
     ...
     */
    generateRequirementsFromData(topic, data) {
        const requirements = [];
        if (data.feature) {
...
//# sourceMappingURL=specification-generator.js.map