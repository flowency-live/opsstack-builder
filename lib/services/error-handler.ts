...
export class ErrorHandler {
   ...
   */
  private static createNetworkErrorNotification(error: Error): UserNotification {
    return {
      message:
        "We're having trouble connecting to the server. Your messages are being saved locally and will be sent when the connection is restored.",
      severity: 'warning',
      actions: [
        {
          label: 'Retry Now',
   ...
   */
  private static createLLMErrorNotification(error: Error): UserNotification {
    return {
      message:
        "I'm having trouble processing your request right now. This could be due to high demand. Your information has been saved, and you can try again in a moment.",
      severity: 'warning',
      actions: [
        {
          label: 'Try Again',
  ...
  private static createPersistenceErrorNotification(
    ...
    return {
      message:
        "We're having trouble saving your progress. Don't worry - your information is stored locally. We'll keep trying to sync it to the server.",
      severity: 'error',
      actions: [
        {
          label: 'Retry Save',
